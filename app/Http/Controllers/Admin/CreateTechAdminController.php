...
class CreateTechAdminController extends Controller
{
    ...
    /**
     * Show the form for creating a new tech admin user.
     * Only Tech Admin can access this page.
     */
    public function create()
    {
        ...
        // Check rate limit
        $rateLimitCheck = $this->rateLimitService->checkRateLimit($user, 'tech_admin');
        
        return view('admin.create-tech-admin', [
            'rateLimitInfo' => $rateLimitCheck,
        ]);
    }

    /**
     * Store a newly created tech admin user.
     * Only Tech Admin can create tech admin users.
     */
    public function store(Request $request)
    {
        ...
        // Check authorization - only tech_admin can access
        if (!$user || !$user->isTechAdmin()) {
            abort(403, 'Only Technical Admins can create tech admin users.');
        }
        ...
        // Force tech_admin role - Tech Admin can ONLY create Tech Admin
        $validated['role'] = 'tech_admin';
        ...
        $this->rateLimitService->logCreation($user, $createdUser, 'tech_admin');

        return redirect()
            ->route('admin.create-tech-admin')
            ->with('success', "Technical Admin user '{$createdUser->username}' has been created successfully.");
    }
}
